...
class IconContainer extends React.Component {
	...
	render() {
		...
		let iconCollection = iconLst.map(
			(url, index) => {
				let active;
				if(this.props.activeUrls.includes(url)) {
					active = true;
				} else {
					active = false;
				}
...
class LeasureBox extends React.Component {
	...
	submitChanges(event) {
		...
		let listIconSelected = [];
		for(let index in listIcon) {
			let anIcon = listIcon[index];
			if(anIcon.className == "leasureIconClicked") {
				newIndexArray.push(index);
				listIconSelected.push(iconLst[index]);
			}
		}
		// On applique la nouvelle selection au conteneur des "passions"
		this.props.propagateLeisureChanges(listIconSelected);
